...
function animate() {
	...
	if (mediaTracks["beat"] && mediaTracks["beat"].length > actualTrack["audio"]) { //beats are locked with audio 1:1 -> we need to check that beat track is defined for the audio currently played (in case it is not, do not show/play beats)
		...
		// draw the text message {"fromTime": 5.500, "toTime": 9.500, "message": "TEST MESSAGE #2"}
		if (mediaTracks["beat"][actualTrack["audio"]].beatData.messages.length > 0 &&
			mediaTracks["beat"][actualTrack["audio"]].beatData.messages[nearestMessageIndex].fromTime < actualTime + latency &&
			mediaTracks["beat"][actualTrack["audio"]].beatData.messages[nearestMessageIndex].toTime > actualTime + latency
		) {
			//console.log("DEBUG: Display Message Index " + nearestMessageIndex);
			c.font = container.height * 0.8 + "px sans-serif"; //"48px serif";
			c.textBaseline = "middle";
			c.fillStyle = 'rgba(255,0,0,0.75)';
			c.fillText(mediaTracks["beat"][actualTrack["audio"]].beatData.messages[nearestMessageIndex].message, 10, container.height / 2);
		} else if (mediaTracks["beat"][actualTrack["audio"]].beatData.messages.length > 0 &&
			mediaTracks["beat"][actualTrack["audio"]].beatData.messages[nearestMessageIndex].toTime <= actualTime + latency &&
			mediaTracks["beat"][actualTrack["audio"]].beatData.messages.length > nearestMessageIndex + 1
		) {
			nearestMessageIndex++;
			//console.log("DEBUG: Nearest Message Index Raised to " + nearestMessageIndex);
		}
